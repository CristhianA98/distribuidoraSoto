...
export const ProductCardEdit = ({ id, imagenes, descripcion, nombre }) => {
    return (
        <>
                    ...
                    <div className="card rounded-0">
                        ...
                        <div className="card-img-overlay rounded-0 product-overlay d-flex align-items-center justify-content-center">
                            <ul className="list-unstyled">
                                <li><Link to={'/admin/listado/editar' + id} className="btn btn-light text-main mt-3"><i className="fa fa-edit fa-3x" /></Link></li>
                            </ul>
                        </div>
                    </div>
                    <div className="card-body">
                        <div className='text-overflow'>
                            <Link to={'/admin/listado/editar' + id} className="h3 text-decoration-none fw-bold" title={nombre}>{nombre}</Link>
                        </div>
                        <ul className="w-100 list-unstyled d-flex justify-content-between mb-0">
                            <div className='cursor text-overflow' title={descripcion}>
                                <Link to={'/admin/listado/editar' + id} className="fs-6 text-decoration-none" title={descripcion}>{descripcion}</Link>
                            </div>
                            <li className="pt-4">
